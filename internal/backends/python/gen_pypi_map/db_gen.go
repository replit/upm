...
func GenerateDB(pkg string, outputFilePath string, cache map[string]PackageInfo, bqFilePath string, pkgsLegacyFile string) error {
	...
	create table pypi_packages (package_name text primary key, version text, summary text, download_count number);
	`)
	if err != nil {
		return err
	}
	...
	for moduleName, candidates := range moduleToPackageList {
		if guess, reason, guessable := GuessPackage(moduleName, candidates, downloadStats); guessable {
			...
			stmt, err = db.Prepare(`
			insert into pypi_packages (package_name, version, summary, download_count)
			values (?, ?, ?, ?)
			...
			_, err = stmt.Exec(guess.Name, guess.Version, guess.Summary, downloadCount)
			if err != nil {
